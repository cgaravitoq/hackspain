...
def build(dataset: Dataset, out_dir: Path, seed: int) -> dict[str, Any]:
    ...
    debt_by_company = dict(
        dataset.debt.group_by("company_id").agg(pl.col("outstanding").sum()).iter_rows()
    )
    ...
    company_group = dict(zip(dataset.companies["company_id"].to_list(), dataset.companies["group_id"].to_list(), strict=True))
    company_currency = dict(zip(dataset.companies["company_id"].to_list(), dataset.companies["currency"].to_list(), strict=True))

    company_rows: dict[str, list[dict[str, Any]]] = {}
    for (company_id,), frame in scored.group_by("company_id", maintain_order=True):
        rows = frame.to_dicts()
        state_list = states([r["momentum"] for r in rows], [r["level"] for r in rows], [r["threshold"] for r in rows])
    ...
    for company_id, rows in company_rows.items():
        ...
        for row in rows:
            ...
            entry = {
                ...
                "drivers": d.drivers(row, previous, facts),
                ...
                "events": {
                    "E1": row["e1"],
                    "E2": e2_months.get(company_id) is not None and month_key(e2_months[company_id]) == month_key(row["month"]),
                    "E3": row["e3"] > 0,
                    "E4": row["e4"],
                },
            }
        ...
        if before and latest["score"] is not None and before["score"] is not None:
    ...
    unscorable = [
        {
            ...
            "debt_outstanding": round(debt_by_company.get(company_id, 0.0), 2),
            "invoice_facts": invoice_facts.get(company_id, {}),
            "latest": {"month": None, "score": None, "level": None, "momentum": None, "state": NOT_EVALUABLE, "confidence": "none"},
        }
        for company_id, group_id in company_group.items()
        if company_id not in company_rows
    ]
    companies_out.extend(unscorable)
        ...
        groups_out.append(
            {
                ...
                "debt_share_top": round(biggest_debtor["debt_outstanding"] / total_debt, 3) if biggest_debtor and total_debt else None,
                "tension": tension,
                "members": [
                    {
    ...
    _write(out_dir / "meta.json", {"state_labels": STATE_LABELS, "latest_month": max(c["latest"]["month"] or "" for c in companies_out), "holdout_groups": sorted(holdout_groups)})
    return {
