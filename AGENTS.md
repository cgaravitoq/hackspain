...
| `bun --filter @hackspain/agent load -- --local` | Validates `pipeline/artifacts` with the shared schemas, writes `artifacts/xray.sql`, applies migrations and loads the local D1 (`-- --artifacts <dir>` reads and writes another folder, default `../../pipeline/artifacts` relative to `apps/agent`; `-- --env staging` targets staging) |
...
- **Every implementation ships with an end-to-end check of the product on real data, quoted in the PR body.** Pipeline and agent work runs the real command or route (`uv run xray ...` on the Embat CSVs, `curl` against the locally loaded D1, a real MCP call). UI work starts `bun run dev`, opens the product in a new tab of Orca's embedded browser and drives it with Orca computer use when the browser automation is not enough. Never the user's own browser. Reviewers check that the evidence exists and can be re-derived.
