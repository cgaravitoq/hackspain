...
def table(headers, rows):
    ...
    return '\n'.join(['| ' + ' | '.join(headers) + ' |', '| ' + ' | '.join(['---'] * len(headers)) + ' |'] + ['| ' + ' | '.join(map(escape, row)) + ' |' for row in rows])


def main():
    audit = json.loads((OUT / 'relationship_audit.json').read_text())
    validation = json.loads((OUT / 'relationship_validation.json').read_text())
    companies = {r['company_id']: r for r in read_csv(DATA / 'companies.csv')}
    groups = read_csv(DATA / 'groups.csv')
    structured = defaultdict(set)
    text = defaultdict(set)
    cp_source = defaultdict(dict)
    incidence_rows = read_csv(OUT / 'relationship_incidence.csv')
    for r in incidence_rows:
        cp, company = r['counterparty_id'], r['company_id']
        (structured if r['evidence_kind'] == 'column' else text)[cp].add(company)
        cp_source[cp][company] = cp_source[cp].get(company, 0) | (1 if r['source'] == 'transactions' else 2)
    types = Counter()
    layer_examples = {}
    edge_fields = ['company_a', 'company_b', 'relationship_type', 'weight', 'direction', 'source_file']
    with (OUT / 'graph_edges.csv').open('w', newline='', encoding='utf-8') as stream:
        writer = csv.DictWriter(stream, fieldnames=edge_fields)
        writer.writeheader()
        for kind in ['shared_counterparty_structured', 'shared_counterparty_mention', 'shared_named_bank', 'shared_bank_service']:
            weights = Counter()
            masks = defaultdict(int)
            if kind.startswith('shared_counterparty'):
                for cp in sorted(structured.keys() | text.keys()):
                    members = structured[cp] if kind == 'shared_counterparty_structured' else structured[cp] | text[cp]
                    for a, b in combinations(sorted(members), 2):
                        if kind == 'shared_counterparty_mention' and a not in text[cp] and b not in text[cp]:
                            continue
                        weights[a, b] += 1
                        if kind == 'shared_counterparty_structured':
                            mask = 0
                            for r in incidence_rows:
                                if r['counterparty_id'] == cp and r['evidence_kind'] == 'column' and r['company_id'] in [a, b]:
                                    mask |= 1 if r['source'] == 'transactions' else 2
                            masks[a, b] |= mask
                        else:
                            masks[a, b] |= cp_source[cp][a] | cp_source[cp][b]
                source_names = ['transactions.csv', 'invoices.csv']
            else:
                members_by_value = defaultdict(set)
                source_by_value = defaultdict(dict)
                for file_index, file_name in enumerate(['banking_products.csv', 'debt_products.csv']):
                    for r in read_csv(DATA / file_name):
                        field = 'bank_name' if kind == 'shared_named_bank' else 'service'
                        value, company = r[field], r['company_id']
                        if not value or value in ['Other (customer-defined)', 'custom']:
                            continue
                        members_by_value[value].add(company)
                        source_by_value[value][company] = source_by_value[value].get(company, 0) | (1 << file_index)
                for value, members in members_by_value.items():
                    for a, b in combinations(sorted(members), 2):
                        weights[a, b] += 1
                        masks[a, b] |= source_by_value[value][a] | source_by_value[value][b]
                source_names = ['banking_products.csv', 'debt_products.csv']
            for (a, b), weight in sorted(weights.items()):
                row = {'company_a': a, 'company_b': b, 'relationship_type': kind, 'weight': weight,
                       'direction': 'undirected', 'source_file': ';'.join(name for i, name in enumerate(source_names) if masks[a, b] & (1 << i))}
                assert a in companies and b in companies and a < b and weight > 0
                writer.writerow(row)
                types[kind] += 1
                layer_examples.setdefault(kind, row)
    write_csv('graph_memberships.csv', ['company_id', 'group_id', 'relationship_type', 'weight', 'direction', 'source_file'],
              ({'company_id': cid, 'group_id': r['group_id'], 'relationship_type': 'member_of_group', 'weight': 1, 'direction': 'company_to_group', 'source_file': 'companies.csv;groups.csv'} for cid, r in sorted(companies.items())))
    write_csv('graph_groups.csv', list(groups[0]), sorted(groups, key=lambda r: r['group_id']))
    write_csv('direct_transaction_pairs.csv', ['company_a', 'company_b', 'transaction_count', 'total_amount'], [])
    write_csv('direct_invoice_pairs.csv', ['company_a', 'company_b', 'invoice_count', 'total_amount'], [])
    manifest = {'edge_rows': sum(types.values()), 'edge_counts': dict(types), 'node_rows': len(companies), 'group_rows': len(groups), 'membership_rows': len(companies), 'layer_examples': layer_examples}
    (OUT / 'graph_manifest.json').write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + '\n')
    score = audit['scores']
    comparison = score['group_comparison']
    tx = audit['transactions']
    inv = audit['invoices']
    shared = audit['shared_counterparties']
    lines = [f'''# Relaciones entre empresas — Investigación desde cero
## Fecha: 2026-09-19

Investigación del contenido completo, no de una muestra de filas. Datos originales: `{DATA}`. Puntuaciones: `/home/juan/hackspain/credit-scoring/results-v2-agent6-20260919/companies/`. El diccionario se leyó antes de analizar las tablas; sus definiciones se reproducen al final. El dataset es **sintético**.
...
### RESUMEN FINAL
''']
    lines.append(table(['Tipo', 'Pares', 'Fuente', 'Usable para grafo'], [
        ...
        ['Mención compartida, al menos un extremo textual', n(types['shared_counterparty_mention']), 'Columna/texto de ambos ficheros', 'Sí, opcional; rol económico incierto'],
...
**Importante:** no cargar todas las capas como si fueran vínculos comerciales. Aplicar el filtro de tipo antes de visualizarlas. Las filas de `graph_edges.csv` incluyen capas opcionales deliberadamente diferenciadas; no son {n(sum(types.values()))} relaciones económicas independientes.
''')
    lines.append(table(['relationship_type', 'Filas', 'Definición exacta de weight', 'Activación recomendada'], [
        ['shared_counterparty_structured', n(types['shared_counterparty_structured']), 'IDs comunes en counterparty_id de cualquiera de los dos ficheros; cada ID una vez', 'Sí'],
        ['shared_counterparty_mention', n(types['shared_counterparty_mention']), 'IDs comunes con al menos un extremo apoyado por description/concept; cada ID una vez', 'No, exploración textual'],
        ['shared_named_bank', n(types['shared_named_bank']), 'bank_name literales comunes, excluye vacío y Other (customer-defined)', 'No, exposición a proveedor'],
        ['shared_bank_service', n(types['shared_bank_service']), 'service literales comunes, excluye vacío y custom', 'No, servicio/proveedor'],
    ]))
    lines.append('''
...
''')
    lines.append(table(['Fichero', 'Filas reales', 'Columnas exactas'], [[name, n(info['rows']), ', '.join('`' + col + '`' for col in info['columns'])] for name, info in audit['files'].items()]))
    for name, info in audit['files'].items():
        lines.append(f'\n#### {name}\n\nFilas: **{n(info["rows"])}**. Valores vacíos por columna:\n')
        lines.append(table(['Columna', 'Vacíos'], [[col, n(info['missing'].get(col, 0))] for col in info['columns']]))
        lines.append('\nEjemplo de fila real:\n```json\n' + json.dumps(info['examples'][0], ensure_ascii=False, indent=2) + '\n```')
    lines.append('''
...
''')
    for field, values in audit['groups']['unique_values'].items():
        lines.append(f'\n**{field}**: {len(values)} valores distintos, incluido vacío si aparece.\n```text\n' + '\n'.join(json.dumps(v, ensure_ascii=False) for v in values) + '\n```')
    lines.append('\nFrecuencias de tamaño de grupo:\n' + table(['Empresas del grupo', 'Grupos'], sorted(audit['groups']['distributions']['n_companies_in_sample'].items(), key=lambda item: int(item[0]))))
    lines.append('\nFrecuencias de ERP de grupo:\n' + table(['ERP (valor literal)', 'Grupos'], [(json.dumps(k, ensure_ascii=False), v) for k, v in audit['groups']['distributions']['erp'].items()]))
    lines.append('\n### ANEXO C — Atributos de empresa y tipos de documento\n')
    for field, values in audit['company_attributes'].items():
        lines.append(f'\n**companies.{field}**, {len(values)} valores incluyendo vacío:\n' + table(['Valor literal', 'Empresas'], [(json.dumps(k, ensure_ascii=False), v) for k, v in values.items()]))
    lines.append('\n**invoices.document_type**:\n' + table(['Tipo', 'Filas'], inv['categorical_values']['document_type'].items()))
    lines.append('\n**Tipos de producto en ambos catálogos**:\n' + table(['Tipo', 'Filas'], audit['products']['type_counts'].items()))
